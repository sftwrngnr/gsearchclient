...
func NewCignacrawl() *Cignacrawl {
	pw, err := playwright.Run()
	if err != nil {
		log.Fatalf("Failed to start Cignacrawl: %v", err)
	}
	return &Cignacrawl{pw: pw}
}

func (dc *Cignacrawl) Init() (err error) {
...
func (dc *Cignacrawl) Run(zc string, zcid uint) (err error) {
	dc.zcid = zcid
...
func (dc *Cignacrawl) AcceptCookies() {
...
func (dc *Cignacrawl) processDentistSearchResults(ent playwright.Locator, disp playwright.Locator) (err error) {
...
func (dc *Cignacrawl) ProcessIndivDentistBlock() {
	var (
		dName  string
		dJob   string
		dAddy  string
	...
	dberr := system.GetSystemParams().Dbc.CreateCignaData(dName, dJob, dAddy, dPhone, dEmail, dc.zcid)
	if dberr != nil {
		fmt.Printf("Couldn't create cigna data: %v\n", dberr)
	}
	// Create
}
